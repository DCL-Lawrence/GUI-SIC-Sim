...
// SIC-Simulator.cpp: 定義應用程式的類別表現方式。
//
...
BEGIN_MESSAGE_MAP(CSICSimulatorApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CSICSimulatorApp::OnAppAbout)
	// 依據文件命令的標準檔案
...
CSICSimulatorApp::CSICSimulatorApp() noexcept
{
	...
	SetAppID(_T("SICSimulator.AppID.NoVersion"));

	// TODO:  在此加入建構程式碼，
	// 將所有重要的初始設定加入 InitInstance 中
}

// 唯一一個 CSICSimulatorApp 物件

CSICSimulatorApp theApp;


// CSICSimulatorApp 初始化
...
int CSICSimulatorApp::ExitInstance()
{
	// TODO: 處理其他您已經加入的資源
	AfxOleTerm(FALSE);
